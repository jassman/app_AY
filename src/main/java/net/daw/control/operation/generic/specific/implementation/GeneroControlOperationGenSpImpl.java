...
public class GeneroControlOperationGenSpImpl extends ControlOperationGenImpl {
    ...
    public GeneroControlOperationGenSpImpl(HttpServletRequest request) throws ClassNotFoundException, NoSuchMethodException, InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, Exception {
        super(request);
        try {
            oGeneroService = new GeneroServiceGenSpImpl(parameterCooker.prepareObject(request), connection);
        } catch (Exception ex) {
            ExceptionBooster.boost(new Exception(this.getClass().getName() + ":GeneroControlOperationSpImpl ERROR: " + ex.getMessage()));
        }
    }
    ...
    public String getcount(HttpServletRequest request) throws Exception {
        String result = null;
        try {
            result = oGeneroService.getcount();
            closeDB();
        } catch (Exception ex) {
