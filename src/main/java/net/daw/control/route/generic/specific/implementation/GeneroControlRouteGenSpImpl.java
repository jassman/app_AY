...
public class GeneroControlRouteGenSpImpl extends ControlRouteGenImpl {

    @Override
    public String execute(HttpServletRequest request, ControlOperationInterface oControl) throws Exception {
        GeneroControlOperationGenSpImpl oGeneroControl = (GeneroControlOperationGenSpImpl) oControl;
        String operation = parameterCooker.prepareOperation(request);
        String jsonResult = "";
        try {
            switch (operation) {
                case "getall":
                    jsonResult = oGeneroControl.getall(request);
                    break;
                case "getcount":
                    jsonResult = oGeneroControl.getcount(request);
                    break;
                default:
